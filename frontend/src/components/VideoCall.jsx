...
const VideoCall = ({ callId, isHelper, onEndCall }) => {
    ...
    useEffect(() => {
        ...
        const checkRemoteStream = setInterval(() => {
            if (!remoteStreamActive && isCallActive) {
                console.log('Remote stream not detected, reconnecting WebSocket...');
                if (wsRef.current) wsRef.current.close(); // Trigger reconnect
            } else {
    ...
    }, [isAuthenticated, callId, isHelper, onEndCall, remoteStreamActive]);
    ...
    return (
        <div className="video-call-container d-flex flex-column justify-content-center align-items-center" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', zIndex: 1000 }}>
            ...
            <div className="button-group" style={{ position: 'absolute', bottom: '20px', left: '50%', transform: 'translateX(-50%)' }}>
                ...
                <Button variant="warning" onClick={retrySignaling} className="mx-2">
                    Retry Signaling
                </Button>
            </div>
        </div>
