...
const Navbar = () => {
      ...
      websocket.onmessage = (e) => {
        ...
        if (data.type === "notification") {
          ...
          if (data.notification.description.includes("started a video call")) {
            const callIdMatch =
              data.notification.description.match(/call ID (\d+)/);
            if (callIdMatch) setCallId(callIdMatch[1]);
          }
        }
      };
      websocket.onerror = (e) =>
  ...
  const uniqueNotifications = Array.from(
    new Map(
      notifications.map((n) => [
        `${n.amount}-${n.description}-${n.timestamp}`,
        n,
      ])
    ).values()
  ).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));

  const handleJoinCall = (id) => setCallId(id);
                    ...
                    <li key={i} className="dropdown-item">
                      ...
                      {n.description.includes("started a video call") && (
                        <Button
                          ...
                          onClick={() =>
                            handleJoinCall(
                              n.description.match(/call ID (\d+)/)[1]
                            )
                          }
                        >
                          Join
